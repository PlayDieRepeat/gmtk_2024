...
func _physics_process(delta: float) -> void:
	%Pivot.rotation.x -= _mouse_direction.y * delta
	%Pivot.rotation.x = clamp(rotation.x, top_clamp, bottom_clamp)
	%Pivot.rotation.y -= _mouse_direction.x * delta
	%Pivot.rotation.y = clamp(rotation.y, -side_clamp, side_clamp)
